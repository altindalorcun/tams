...
- [ ] **1.1** `services/rule-service/src/main/resources/db/migration/V6__add_code_to_departments.sql` dosyasını oluştur:
  ```sql
...
- [ ] **1.2** `Department.java` entity'sine `code` alanı ekle:
...
- [ ] **1.3** `DepartmentResponse.java` record'una `String code` alanı ekle

- [ ] **1.4** `CreateDepartmentRequest.java` record'una `@NotBlank @Size(max = 20) String code` ekle

- [ ] **1.5** `UpdateDepartmentRequest.java` record'una `@NotBlank @Size(max = 20) String code` ekle

- [ ] **1.6** `DepartmentService.java` içinde `create()` ve `update()` metodlarına `code` için unique kontrol ekle (mevcut `name` kontrolüne paralel olarak)

- [ ] **1.7** `DepartmentController.java` — `create()` ve `update()` metodlarında service'e `code` iletildiğini doğrula (gerekirse güncelle)
...
- [ ] **2.1** `auth-service` Flyway migration numarasını kontrol et (`db/migration/` klasöründeki son V numarası neyse bir sonraki). `V?__add_student_number_to_users.sql` oluştur:
  ```sql
  ALTER TABLE users ADD COLUMN student_number VARCHAR(20) UNIQUE;
  ```

- [ ] **2.2** `User.java` entity'sine nullable `studentNumber` alanı ekle:
  - `@Column(name = "student_number", unique = true, length = 20) private String studentNumber;`

- [ ] **2.3** `RegisterRequest.java` record'una `String studentNumber` (opsiyonel, `@Nullable`) ekle

- [ ] **2.4** `AuthServiceImpl.register()` metoduna validasyon ekle:
...
- [ ] **2.5** `JwtUtil.java` → `generateAccessToken()` metoduna:
...
- [ ] **2.6** `AuthResponse.java` record'una `String studentNumber` ekle (student için JWT response'a da dönmek gerekebilir)
...
- [ ] **3.1.1** `V4__add_department_name_gpa_to_analysis_results.sql` oluştur:
  ```sql
...
- [ ] **3.1.2** `V5__create_category_results_table.sql` oluştur:
  ```sql
...
- [ ] **3.1.3** `V6__drop_deficiencies_table.sql` oluştur:
  ```sql
...
- [ ] **3.2.1** `domain/CategoryResult.java` entity'si oluştur:
...
- [ ] **3.2.2** `AnalysisResult.java` entity'sini güncelle:
...
- [ ] **3.2.3** `domain/Deficiency.java` dosyasını sil

### 3.3 Repository

Ön koşul: 3.2 tamamlandı

- [ ] **3.3.1** `repository/CategoryResultRepository.java` interface'i oluştur:
  ```java
...
- [ ] **3.4.1** `service/GpaCalculator.java` `@Component`'i oluştur. Hacettepe not skalası:
...
- [ ] **3.4.2** `service/dto/EngineResult.java` record'una `BigDecimal gpa` alanı ekle

- [ ] **3.4.3** `service/GraduationEngine.java` → `evaluate()` metodunda `GpaCalculator` çağır ve `EngineResult`'a `gpa` ekle

### 3.5 Servis Katmanı

Ön koşul: 3.2, 3.3, 3.4 tamamlandı

- [ ] **3.5.1** `service/ResultService.java` → `completeResult()` metodunu güncelle:
...
- [ ] **3.6.1** `dto/CategoryResultResponse.java` record'u oluştur:
  ```java
...
- [ ] **3.6.2** `dto/AnalysisResultDetailResponse.java` güncelle:
  - `List<DeficiencyResponse> deficiencies` → `List<CategoryResultResponse> categoryResults`
  - `String departmentName` ekle
  - `BigDecimal gpa` ekle

- [ ] **3.6.3** `dto/AnalysisResultSummaryResponse.java` güncelle:
  - `String departmentName` ekle
  - `BigDecimal gpa` ekle

- [ ] **3.6.4** `dto/DeficiencyResponse.java` dosyasını sil

### 3.7 Controller

Ön koşul: 3.6 tamamlandı

- [ ] **3.7.1** `controller/ResultController.java` → yeni endpoint ekle:
  ```
  GET /api/v1/results/by-job/{jobId}  (TEACHER rolü)
  ```
  Öğretmen PDF yükledikten sonra `jobId` ile direkt sonuca erişebilir.

- [ ] **3.7.2** `controller/ResultController.java` → `getMyResult()` metodunu güncelle:
...
- [ ] **3.8.1** `service/ResultQueryService.java` güncelle:
...
- [ ] **4.1.1** `Department` interface'ini güncelle:
  - `code: string` ekle
  - `description?: string` ekle
  - `createdAt?: string`, `updatedAt?: string` ekle

- [ ] **4.1.2** `Course` interface'ini güncelle:
  - `name` → `courseName`
  - `credits` → `credit`

- [ ] **4.1.3** `DepartmentCourse` interface'ini güncelle:
  - `name` → `courseName`
  - `credits` → `credit`

- [ ] **4.1.4** `CategoryCourse` interface'ini güncelle:
  - `name` → `courseName`
  - `credits` → `credit`

- [ ] **4.1.5** `Category` interface'ini güncelle:
  - `description?: string` ekle

- [ ] **4.1.6** Request tiplerini güncelle:
...
- [ ] **4.2.1** Ders oluşturma/güncelleme request body'lerinde `name` → `courseName`, `credits` → `credit` rename et

- [ ] **4.2.2** Bölüm oluşturma/güncelleme request body'lerine `code` alanını ekle

### 4.3 Admin Bileşenleri

Ön koşul: 4.1, 4.2 tamamlandı

- [ ] **4.3.1** `DepartmentsTab.tsx` güncelle:
  - Oluşturma/düzenleme formuna `code` (zorunlu) ve `description` (opsiyonel) alanları ekle
  - Tablo kolonuna `code` ekle

- [ ] **4.3.2** `CoursesTab.tsx` güncelle:
  - `name` → `courseName`, `credits` → `credit` field referanslarını düzelt

- [ ] **4.3.3** `CategoriesTab.tsx` güncelle:
  - `name` → `courseName`, `credits` → `credit` field referanslarını düzelt
...
- [ ] **4.4.1** `Deficiency` interface'ini sil

- [ ] **4.4.2** `CategoryResult` interface'ini backend `CategoryResultResponse` ile hizala:
  ```typescript
...
- [ ] **4.4.3** `AnalysisResult` interface'ini güncelle:
...
- [ ] **4.4.4** `AnalysisResultSummary` interface'ini güncelle:
...
- [ ] **4.5.1** `getMyResult()` fonksiyonundan `?studentRef=` query param'ını kaldır (backend artık JWT'den alıyor)

- [ ] **4.5.2** `getResultByJobId(jobId: string)` fonksiyonu ekle:
  ```typescript
...
- [ ] **4.6.3** `TeacherPage.tsx` güncelle:
...
- [ ] **D.1** rule-service servisini yeniden başlat, Flyway migrasyonlarının hatasız çalıştığını doğrula:
  ```bash
