...
interface AuthStore extends AuthState {
  setAuth: (token: string, mustChangePassword: boolean, username: string) => void;
  clearAuth: () => void;
}
 ...
 * mustChangePassword and username are also persisted so a refresh mid-flow keeps state.
 */
export const useAuthStore = create<AuthStore>((set) => ({
  accessToken: initialParsed ? storedToken : null,
  role: initialParsed?.role ?? null,
  userId: initialParsed?.userId ?? null,
  username: initialParsed ? storedUsername : null,
  mustChangePassword: initialParsed ? storedMustChange : false,

  setAuth: (token: string, mustChangePassword: boolean, username: string) => {
